...
public final class ActiveTagGeneration {
    ...
    public ActiveTagGeneration(UUID id, Instant at, Map<ResourceKey<? extends Registry<?>>, Map<TagKey<?>, List<Holder<?>>>> registries, RegistryAccess access) {
        this(id, at, registries, access, java.util.Map.of());
    }
    public ActiveTagGeneration(UUID id, Instant at, Map<ResourceKey<? extends Registry<?>>, Map<TagKey<?>, List<Holder<?>>>> registries, RegistryAccess access, Map<ResourceKey<? extends Registry<?>>, Set<ResourceLocation>> universe) {
        ...
        for (var entry : copy.entrySet()) {
            ...
            Set<ResourceLocation> ids = new java.util.TreeSet<>(universe.getOrDefault(entry.getKey(), Set.of()));
            entry.getValue().keySet().stream().map(TagKey::location).forEach(ids::add);
            for (ResourceLocation idLocation : ids) {
                var matching = entry.getValue().entrySet().stream().filter(e -> e.getKey().location().equals(idLocation)).findFirst();
                if (matching.isEmpty()) { tags.put(idLocation, new ActiveTagSnapshot(TagState.MISSING, List.of())); continue; }
                var tag = matching.get();
