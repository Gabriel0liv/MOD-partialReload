...
public final class PartialReloadService {
    ...
    public synchronized void processTagRecipeSafePoint(MinecraftServer server) {
        ...
        try {
            ...
            Map<ResourceKey<? extends Registry<?>>, Map<TagKey<?>, List<Holder<?>>>> candidate=buildCandidateBindings(server.registryAccess(), artifact.preparedTags(), registriesToMutate); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.CANDIDATE_BUILT, TagRecipeTransactionStatus.BUILDING_BINDINGS, "CANDIDATE_BINDINGS_BUILT");
            List<Recipe<?>> candidateRecipes=artifact.preparedRecipes().recipesById().values().stream().map(PreparedRecipe::recipe).toList(); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.CANDIDATE_BUILT, TagRecipeTransactionStatus.BUILDING_BINDINGS, "CANDIDATE_RECIPES_BUILT:" + candidateRecipes.size());
            ActiveTagRecipeGeneration previous=captureTagRecipeGeneration(server, registriesToMutate); tx.previousGeneration(previous); retainedTagRecipeGeneration=previous; tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.CANDIDATE_BUILT, TagRecipeTransactionStatus.READY_TO_COMMIT, "PREVIOUS_GENERATION_CAPTURED:" + previous.generationId());
            stateMachine.transitionTo(PartialReloadState.COMMITTING);
            tx.status(TagRecipeTransactionStatus.BINDING_TAGS);
            int bindIndex=0; for (var e:candidate.entrySet()) { tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.TAG_BIND_STARTED, TagRecipeTransactionStatus.BINDING_TAGS, "TAG_REGISTRY_BIND_STARTED:" + e.getKey().location()); if(bindIndex==0) TagRecipeFaultInjection.hit(TagRecipeFaultPoint.BEFORE_FIRST_TAG_BIND); if(bindIndex==1) TagRecipeFaultInjection.hit(TagRecipeFaultPoint.BEFORE_SECOND_TAG_BIND); bind(server.registryAccess(),e.getKey(),e.getValue()); tx.tagRegistryMutated(e.getKey()); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.TAG_BOUND, TagRecipeTransactionStatus.BINDING_TAGS, "TAG_REGISTRY_BOUND:" + e.getKey().location()); if(bindIndex==0) TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_FIRST_TAG_BIND); bindIndex++; }
            ...
            tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.RECIPES_PUBLISHED, TagRecipeTransactionStatus.PUBLISHING_RECIPES, "RECIPE_PUBLICATION_STARTED:" + candidateRecipes.size()); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.BEFORE_RECIPE_PUBLICATION); server.getRecipeManager().replaceRecipes(candidateRecipes); tx.recipeMutationOccurred(true); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.RECIPES_PUBLISHED, TagRecipeTransactionStatus.PUBLISHING_RECIPES, "RECIPES_PUBLISHED:" + candidateRecipes.size()); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_RECIPE_PUBLICATION);
            tx.status(TagRecipeTransactionStatus.INVALIDATING_CACHES); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.INGREDIENTS_INVALIDATED, TagRecipeTransactionStatus.INVALIDATING_CACHES, "INGREDIENT_INVALIDATION_STARTED"); Ingredient.invalidateAll(); tx.ingredientInvalidationOccurred(true); tx.ingredientCommitInvalidations(tx.ingredientCommitInvalidations()+1); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.INGREDIENTS_INVALIDATED, TagRecipeTransactionStatus.INVALIDATING_CACHES, "INGREDIENTS_INVALIDATED"); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_INGREDIENT_INVALIDATION);
            tx.status(TagRecipeTransactionStatus.DISPATCHING_EVENTS); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.TAGS_EVENT_DISPATCHED, TagRecipeTransactionStatus.DISPATCHING_EVENTS, "TAGS_EVENT_DISPATCH_STARTED"); if (MinecraftForge.EVENT_BUS.post(new TagsUpdatedEvent(server.registryAccess(), false, false))) throw new IllegalStateException("TAG_UPDATE_EVENT_FAILED"); tx.tagsUpdatedEventDispatched(true); tx.commitTagEvents(tx.commitTagEvents()+1); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.TAGS_EVENT_DISPATCHED, TagRecipeTransactionStatus.DISPATCHING_EVENTS, "TAGS_EVENT_DISPATCHED"); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_TAGS_UPDATED_EVENT);
            ...
            try { TagRecipeFaultInjection.hit(TagRecipeFaultPoint.DURING_ROLLBACK); restoreTagRecipeGeneration(server,tx.previousGeneration(),tx); }
                ...
                tx.failure(lastError);
                tx.status(TagRecipeTransactionStatus.DEGRADED); stateMachine.transitionTo(PartialReloadState.DEGRADED);
            }
        }
    }
    ...
    private void restoreTagRecipeGeneration(MinecraftServer server, ActiveTagRecipeGeneration generation, TagRecipeCommitTransaction tx){
        ...
        tx.status(TagRecipeTransactionStatus.ROLLING_BACK_RECIPES); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.ROLLBACK_STARTED, TagRecipeTransactionStatus.ROLLING_BACK_RECIPES,"ROLLBACK_STARTED"); if (tx.preparationId()==null || tx.recipePublicationOccurred()) { server.getRecipeManager().replaceRecipes(generation.recipes().recipes()); tx.recipeMutationOccurred(true); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.ROLLBACK_RECIPES_RESTORED, TagRecipeTransactionStatus.ROLLING_BACK_RECIPES,"ROLLBACK_RECIPES_RESTORED"); }
        tx.status(TagRecipeTransactionStatus.ROLLING_BACK_TAGS); for(var e:generation.tags().registries().entrySet()) if(tx.mutatedTagRegistries().contains(e.getKey())) { bind(server.registryAccess(),e.getKey(),e.getValue()); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.ROLLBACK_TAG_RESTORED, TagRecipeTransactionStatus.ROLLING_BACK_TAGS,"ROLLBACK_TAG_REGISTRY_RESTORED:" + e.getKey().location()); }
        if (!tx.mutatedTagRegistries().isEmpty() || tx.preparationId()==null || tx.recipePublicationOccurred()) { Ingredient.invalidateAll(); tx.ingredientInvalidationOccurred(true); tx.ingredientRollbackInvalidations(tx.ingredientRollbackInvalidations()+1); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.ROLLBACK_INGREDIENTS_INVALIDATED, TagRecipeTransactionStatus.ROLLBACK_EVENTS,"ROLLBACK_INGREDIENTS_INVALIDATED"); tx.status(TagRecipeTransactionStatus.ROLLBACK_EVENTS); MinecraftForge.EVENT_BUS.post(new TagsUpdatedEvent(server.registryAccess(),false,false)); tx.tagsUpdatedEventDispatched(true); tx.rollbackTagEvents(tx.rollbackTagEvents()+1); tx.event(com.gabriel0liv.partialreload.joint.TagRecipeTransactionEventType.ROLLBACK_TAGS_EVENT_DISPATCHED, TagRecipeTransactionStatus.ROLLBACK_EVENTS,"ROLLBACK_TAGS_EVENT_DISPATCHED"); }
