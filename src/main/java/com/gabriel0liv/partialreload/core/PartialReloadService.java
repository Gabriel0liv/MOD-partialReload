...
public final class PartialReloadService {
    ...
    public synchronized void processTagRecipeSafePoint(MinecraftServer server) {
        ...
        try {
            ...
            tx.status(TagRecipeTransactionStatus.INVALIDATING_CACHES); tx.event(TagRecipeTransactionStatus.INVALIDATING_CACHES, "INGREDIENT_INVALIDATION_STARTED"); Ingredient.invalidateAll(); tx.ingredientInvalidationOccurred(true); tx.ingredientCommitInvalidations(tx.ingredientCommitInvalidations()+1); tx.event(TagRecipeTransactionStatus.INVALIDATING_CACHES, "INGREDIENTS_INVALIDATED"); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_INGREDIENT_INVALIDATION);
            tx.status(TagRecipeTransactionStatus.DISPATCHING_EVENTS); tx.event(TagRecipeTransactionStatus.DISPATCHING_EVENTS, "TAGS_EVENT_DISPATCH_STARTED"); if (MinecraftForge.EVENT_BUS.post(new TagsUpdatedEvent(server.registryAccess(), false, false))) throw new IllegalStateException("TAG_UPDATE_EVENT_FAILED"); tx.tagsUpdatedEventDispatched(true); tx.commitTagEvents(tx.commitTagEvents()+1); tx.event(TagRecipeTransactionStatus.DISPATCHING_EVENTS, "TAGS_EVENT_DISPATCHED"); TagRecipeFaultInjection.hit(TagRecipeFaultPoint.AFTER_TAGS_UPDATED_EVENT);
